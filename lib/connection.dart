    ...
    FlutterBluetoothSerial.instance.getBondedDevices().then((bondedDevices) {
      setState(() {
        devices = bondedDevices
            .map((device) =>
            _DeviceWithAvailability(device, _isDiscovering ? _DeviceAvailability.maybe : _DeviceAvailability.yes, widget.rssi))
            .toList();
      });
    });
  }
  ...
  void _startDiscovery() {
    _discoveryStreamSubscription =
        FlutterBluetoothSerial.instance.startDiscovery().listen((r) {
          setState(() {
            var deviceIndex = devices.indexWhere((device) => device.device == r.device);
            if (deviceIndex != -1) {
